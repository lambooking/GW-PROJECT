...
"""
    ...
    def get_signature_verification_with_details_prompt(self, context: str, max_score: int = 15) -> str:
        ...
        """
        prompt = f"""
你将看到一张或多张包含签字/签章/审核批注的页面图像（通常在封面或前几页）。
...
编制：[姓名]，[YYYY年MM月DD日 或 YYYY-MM-DD]
审核：[姓名]，[YYYY年MM月DD日 或 YYYY-MM-DD]
批准：[姓名]，[YYYY年MM月DD日 或 YYYY-MM-DD]

分数：X/{max_score}
评价：[评估签字角色覆盖度、签章清晰度、日期时序是否合理、与文本描述是否一致]
