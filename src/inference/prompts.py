...
"""
    ...
    def get_signature_verification_with_details_prompt(self, context: str, max_score: int = 15) -> str:
        ...
        """
        prompt = f"""
你将看到包含签字/签章页面的图像（可能有多张，通常在封面或前几页）。

**任务**：逐张图片仔细查找以下签字角色，并提取姓名和日期：
- 编制（或起草、编写）
- 校对
- 审核（或复核）
- 批准（或签发、批准人）
...
**重要提示**：
1. 签字可能分布在不同页面（第1页、第2页、第3页等）
2. 有些页面可能有表格格式的签字栏，包含：编制、校对、审核等
3. 批准人可能单独在最后一页
4. 手写签名通常在对应角色旁边
5. 日期可能写在签名附近，格式如：2024年7月24日、2024-07-24
...
分数：13/15
说明：在第2页找到编制、校对、审核的签字表格，在第3页找到批准人签字，签字清晰完整，日期时序合理。
